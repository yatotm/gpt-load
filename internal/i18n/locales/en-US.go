...
var MessagesEnUS = map[string]string{
	...
	"validation.aggregate_no_stream_timeout_rules":           "Aggregate groups do not support model stream timeout rules",
	"validation.invalid_stream_timeout_rules":                "Invalid model stream timeout rules: {{.error}}",
	...
	"config.stream_first_visible_timeout":                   "Stream First Visible Output Timeout (seconds)",
	"config.stream_first_visible_timeout_desc":              "Maximum time from request dispatch to the first visible streaming output event. Heartbeats and blank frames do not count. If it expires before anything is written to the client, the request is retried.",
	"config.stream_timeout_failure_penalty_multiplier":      "Stream Timeout Failure Penalty Multiplier",
	"config.stream_timeout_failure_penalty_multiplier_desc": "Penalty multiplier applied only to stream first-visible-output timeout failures. 1 counts as one failure, 0.5 means two timeouts equal one failure, and 2 means one timeout counts as two failures.",
	...
	"config.blacklist_threshold_desc":             "Blacklist a key when its accumulated failures within the rolling failure window reach this threshold. Set to 0 to disable.",
	"config.blacklist_window_minutes":             "Failure Window (minutes)",
	"config.blacklist_window_minutes_desc":        "Rolling time window in minutes used to accumulate passive request failures.",
	"config.consecutive_failure_threshold":        "Consecutive Failure Threshold",
	"config.consecutive_failure_threshold_desc":   "Immediately blacklist a key after this many consecutive failures. Set to 0 to disable this rule.",
