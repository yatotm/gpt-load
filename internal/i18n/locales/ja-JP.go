...
var MessagesJaJP = map[string]string{
	...
	"validation.aggregate_no_stream_timeout_rules":           "集約グループはモデル別ストリームタイムアウトルールをサポートしていません",
	"validation.invalid_stream_timeout_rules":                "モデル別ストリームタイムアウトルールが無効です: {{.error}}",
	...
	"config.stream_first_visible_timeout":                   "ストリーム初回可視出力タイムアウト（秒）",
	"config.stream_first_visible_timeout_desc":              "リクエスト送信から最初の可視ストリーム出力イベントまでの最大待機時間です。ハートビートや空フレームは進捗として扱いません。クライアントへ何も書き出す前に超過した場合はリトライします。",
	"config.stream_timeout_failure_penalty_multiplier":      "ストリーム初回可視出力タイムアウト失敗倍率",
	"config.stream_timeout_failure_penalty_multiplier_desc": "ストリーム初回可視出力タイムアウト時にのみ適用される失敗ペナルティ倍率です。1 は 1 回分、0.5 は 2 回で 1 回分、2 は 1 回で 2 回分として扱います。",
	...
	"config.blacklist_threshold_desc":             "失敗集計ウィンドウ内の累積失敗回数がこのしきい値に達したときキーをブラックリスト化します。0で無効。",
	"config.blacklist_window_minutes":             "失敗集計ウィンドウ（分）",
	"config.blacklist_window_minutes_desc":        "受動的なリクエスト失敗を集計するローリング時間ウィンドウ（分）。",
	"config.consecutive_failure_threshold":        "連続失敗ブラックリストしきい値",
	"config.consecutive_failure_threshold_desc":   "この回数だけ連続失敗したキーを即座にブラックリスト化します。0で無効。",
