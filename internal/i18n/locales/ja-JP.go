...
var MessagesJaJP = map[string]string{
	...
	"config.active_probe_enabled":                 "アクティブプローブを有効化",
	"config.active_probe_enabled_desc":            "有効化すると、システムが固定間隔で各キーの接続性を能動的に確認し、失敗率に応じて自動的にブラックリスト化または復旧します。",
	"config.active_probe_interval_seconds":        "アクティブプローブ間隔（秒）",
	"config.active_probe_interval_seconds_desc":   "各キーに対してアクティブプローブを実行する間隔（秒）。",
	"config.active_probe_timeout_seconds":         "アクティブプローブタイムアウト（秒）",
	"config.active_probe_timeout_seconds_desc":    "単一のアクティブプローブ要求のタイムアウト（秒）。タイムアウトは失敗として扱われます。",
	"config.active_probe_window_minutes":          "アクティブプローブ集計ウィンドウ（分）",
	...
	"error.invalid_probe_param_overrides": "無効なアクティブプローブ用パラメーター上書き設定: {{.error}}",
