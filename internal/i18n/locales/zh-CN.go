...
var MessagesZhCN = map[string]string{
	...
	"validation.aggregate_no_stream_timeout_rules":           "聚合分组不支持配置模型首字超时规则",
	"validation.invalid_stream_timeout_rules":                "模型首字超时规则无效: {{.error}}",
	...
	"config.stream_first_visible_timeout":                   "流式首个可见输出超时（秒）",
	"config.stream_first_visible_timeout_desc":              "从请求发出到流式首个可见输出事件的最长等待时间；纯心跳与空白不会计入。若在向客户端写出任何流数据前超时，将触发重试。",
	"config.stream_timeout_failure_penalty_multiplier":      "流式首字超时失败倍率",
	"config.stream_timeout_failure_penalty_multiplier_desc": "仅对流式首个可见输出超时生效的失败惩罚倍率。1 表示按 1 次失败计，0.5 表示两次超时才等于 1 次失败，2 表示一次超时按 2 次失败计。",
	...
	"config.blacklist_threshold_desc":             "在失败统计窗口内，一个 Key 的累计失败次数达到该阈值后进入黑名单，0为不拉黑。",
	"config.blacklist_window_minutes":             "失败统计窗口（分钟）",
	"config.blacklist_window_minutes_desc":        "统计被动失败累计次数的滚动时间窗口（分钟）。",
	"config.consecutive_failure_threshold":        "连续失败拉黑阈值",
	"config.consecutive_failure_threshold_desc":   "一个 Key 连续失败达到该阈值后立即进入黑名单，0为不启用该规则。",
