...
var MessagesZhCN = map[string]string{
	...
	"config.active_probe_enabled":                 "启用主动探测",
	"config.active_probe_enabled_desc":            "启用后，系统会按固定间隔主动探测每个 Key 的连通性，并基于失败率自动拉黑或拉白。",
	"config.active_probe_interval_seconds":        "主动探测间隔（秒）",
	"config.active_probe_interval_seconds_desc":   "系统主动探测每个 Key 的时间间隔（秒）。",
	"config.active_probe_timeout_seconds":         "主动探测超时（秒）",
	"config.active_probe_timeout_seconds_desc":    "主动探测单个 Key 时的超时时间（秒），超时视为失败。",
	"config.active_probe_window_minutes":          "主动探测统计窗口（分钟）",
	...
	"error.invalid_probe_param_overrides": "无效的主动探测参数覆盖配置: {{.error}}",
